...
router.post("/", async (req, res) => {
  const { transcript_id, text, ticker, subsector, datatitle, sentiment, rating } = req.body;
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO annotations (transcript_id, text, ticker, subsector, datatitle, sentiment, rating, created_at)
       ...
       RETURNING *`,
  [transcript_id, text, ticker, subsector, datatitle || null, sentiment, r]
    );
...
router.put("/:id", async (req, res) => {
  const { text, ticker, subsector, datatitle, sentiment, rating } = req.body;
  const { id } = req.params;
  ...
  try {
    const result = await pool.query(
      `UPDATE annotations
       SET text = $1, ticker = $2, subsector = $3, datatitle = $4, sentiment = $5, rating = $6, created_at = NOW()
       WHERE id = $7
       RETURNING *`,
  [text, ticker, subsector, datatitle || null, sentiment, r, id]
    );
